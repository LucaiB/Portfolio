...
mergeSort:
	...
	jge end 		; jump to end
	push rsi		; else push left index
	push rdx 		; push right index
	add rdx, rsi 		; start + end
	shr rdx, 1 		; divide by 2 to get midpoint
	push rdx		; push midpoint
	call mergeSort 		; mergesort on first half
	pop rsi 		; mid = start
	pop rdx 		; end 
	inc rsi 		; mid + 1 
	push rdx 		;
	call mergeSort 		; mergesort on second half
	pop rdx 		; put right index into register
	pop rsi 		; left index into correct register
	mov rcx, rdx 		; right index into correct register
	add rdx, rsi		; start + end
	shr rdx, 1 		; /2 == midpoint
	call merge 		; 
